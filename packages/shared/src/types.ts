...
export type VaultState = Vault & {
  ...
  /**
   * Optional viewing key for the family vault. Present only when the user
   * has pasted one in to decrypt shielded history.
   */
  viewingKeyNk?: string;
};

/**
 * Cloak's mainnet program ID. Mirrors `@cloak.dev/sdk@0.1.5`'s exported
 * `CLOAK_PROGRAM_ID`. Kept here so UI code can render the label without
 * pulling in the whole 1MB SDK bundle. For ACTUAL on-chain calls, prefer
 * importing `CLOAK_PROGRAM_ID` from the SDK directly - it's authoritative
 * if the SDK ever reassigns.
 */
export const CLOAK_MAINNET_PROGRAM_ID =
